...
fn main() {
    ...
    let upper_left = parse_complex(&args[3])
        .expect("ошибка при разборе координат верхнего левого угла");

    let lower_right = parse_complex(&args[4])
        .expect("ошибка при разборе координат нижнего правого угла");
...
fn test_parse_complex() {
    assert_eq!(parse_complex("1.25,0.5"), Some(Complex { re: 1.25, im: 0.5 }));
...
fn test_pixel_to_point() {
    ...
    assert_eq!(result, Complex { re: -0.5, im: -0.5 });
}